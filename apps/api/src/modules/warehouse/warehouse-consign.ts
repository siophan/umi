...
export async function consignPhysicalWarehouseItem(userId: string, routeId: string, price?: number) {
  ...
  return { success: true as const, estimateDays };
}

export async function cancelPhysicalWarehouseConsign(userId: string, routeId: string) {
  const rawId = getRouteIdParam(routeId);
  if (!rawId.startsWith('pw-')) {
    throw new HttpError(400, 'INVALID_ID', '无效的商品 ID');
  }
