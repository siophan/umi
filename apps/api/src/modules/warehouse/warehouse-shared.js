...
export function sanitizeVirtualRow(row) {
    return {
...
export function sanitizePhysicalRow(row) {
    return {
        id: toEntityId(row.id),
        userId: toEntityId(row.user_id),
        productId: toEntityId(row.product_id ?? 0),
...
export function computeEstimateDays(priceYuan, marketPriceYuan) {
...
export function getRouteIdParam(value) {
