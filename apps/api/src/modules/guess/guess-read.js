...
import { buildGuessSummary, getGuessOptionRows, getGuessRows, getGuessVoteRows, getRouteParam, GUESS_ACTIVE, GUESS_PENDING_REVIEW, GUESS_SETTLED, REVIEW_APPROVED, } from './guess-shared';
export async function getGuessList(query) {
    ...
    const params = [REVIEW_APPROVED, GUESS_PENDING_REVIEW, GUESS_ACTIVE, GUESS_SETTLED];
    if (keyword) {
        const like = `%${keyword}%`;
    ...
    return {
        items: rows.map((row) => buildGuessSummary(row, optionsByGuess.get(String(row.id)) || [], voteByGuess.get(String(row.id)) || [])),
    };
}
export async function getGuessDetail(routeParam) {
...
export async function getGuessStats(routeParam) {
    const rows = await getGuessRows('WHERE g.id = ?', [getRouteParam(routeParam)]);
    const row = rows[0];
    if (!row) {
        throw new HttpError(404, 'GUESS_NOT_FOUND', 'Guess not found');
