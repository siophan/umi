...
async function getUserHistoryResult(userId, userName) {
    const db = getDbPool();
...
guessRouter.get('/', async (request, response) => {
    ...
    const items = rows.map((row) => buildGuessSummary(row, optionsByGuess.get(String(row.id)) || [], voteByGuess.get(String(row.id)) || []));
    ok(response, { items });
});
