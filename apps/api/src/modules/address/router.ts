...
import type { AddressPayload } from '@umi/shared';
...
export const addressRouter: ExpressRouter = Router();
...
addressRouter.post(
  ...
  withErrorBoundary(
    ...
    async (request, response) => {
      const user = getRequestUser(request);
      ok(response, await createAddress(user.id, request.body as AddressPayload));
    },
  ),
);
...
addressRouter.put(
  ...
  withErrorBoundary(
    ...
    async (request, response) => {
      const user = getRequestUser(request);
      ok(response, await updateAddress(user.id, String(request.params.id), request.body as AddressPayload));
    },
  ),
);
