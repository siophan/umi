...
function sanitizeChatMessage(row, userId) {
    const senderId = toEntityId(row.sender_id);
    const receiverId = toEntityId(row.receiver_id);
    return {
...
async function upsertConversationSummary(userId, peerId, lastMessage, unreadMode) {
    const db = getDbPool();
    await db.execute(`
...
export async function getChatDetail(userId, peerId) {
    const peer = await getUserSummaryById(peerId);
    if (!peer) {
        throw new Error('聊天对象不存在');
    }
    const db = getDbPool();
    await db.execute(`UPDATE chat_message SET is_read = 1, updated_at = CURRENT_TIMESTAMP(3) WHERE sender_id = ? AND receiver_id = ? AND is_read = 0`, [peerId, userId]);
    await db.execute(`UPDATE chat_conversation SET unread_count = 0, updated_at = CURRENT_TIMESTAMP(3) WHERE user_id = ? AND peer_id = ?`, [userId, peerId]);
    const [rows] = await db.execute(`
    ...
    return {
        ...
        items: rows.map((row) => sanitizeChatMessage(row, userId)),
    };
}
export async function sendChatMessage(userId, peerId, payload) {
    const peer = await getUserSummaryById(peerId);
    if (!peer) {
        throw new Error('聊天对象不存在');
    }
    ...
    const [rows] = await db.execute(`SELECT id, sender_id, receiver_id, content, is_read, created_at FROM chat_message WHERE id = ? LIMIT 1`, [result.insertId]);
    const row = rows[0];
    if (!row) {
