...
function mapEquityLogType(code) {
...
function mapEquitySubType(code) {
...
function mapEquitySubTypeCode(value) {
    if (value === 'category') {
...
function sanitizeEquityAccount(row) {
...
function sanitizeEquityLog(row) {
    return {
        id: toEntityId(row.id),
...
export async function getAdminEquityAccounts(params) {
    ...
    return {
        items: rows.map(sanitizeEquityAccount),
        total,
        page,
        pageSize,
        summary: {
...
export async function getAdminEquityDetail(userId) {
    const db = getDbPool();
    ...
    return {
        ...
        logs: logRows.map(sanitizeEquityLog),
    };
}
export async function adjustAdminEquity(payload) {
    if (!payload.userId) {
    ...
    try {
        ...
        FOR UPDATE
      `, [payload.userId]);
        let accountRow = accountRows[0] ?? null;
        ...
        `, [payload.userId]);
          ...
          LIMIT 1
        `, [payload.userId]);
            accountRow = createdRows[0] ?? null;
        }
        if (!accountRow) {
            throw new HttpError(500, 'ADMIN_EQUITY_ACCOUNT_CREATE_FAILED', '权益金账户初始化失败');
        }
        const currentValue = Number(accountRow[selectedField] ?? 0);
      ...
      `, [amountYuan, totalGrantedIncrement, totalUsedIncrement, accountRow.id]);
        ...
        LIMIT 1
      `, [accountRow.id]);
        const updatedAccountRow = updatedRows[0] ?? null;
        if (!updatedAccountRow) {
            throw new HttpError(500, 'ADMIN_EQUITY_ACCOUNT_UPDATE_FAILED', '权益金账户更新失败');
        }
        const totalBalanceYuan = Number(updatedAccountRow.category_amount ?? 0) +
      ...
      `, [logResult.insertId]);
        ...
        return {
            account: sanitizeEquityAccount(detailRows[0] ?? updatedAccountRow),
            log: sanitizeEquityLog(logRows[0]),
        };
    }
    catch (error) {
