...
function sortByIds(items, ids) {
    const indexes = new Map(ids.map((id, index) => [id, index]));
    return [...items].sort((left, right) => (indexes.get(left.id) ?? Number.MAX_SAFE_INTEGER) -
...
export async function getAdminUserGuesses(userId, page = 1, pageSize = 10) {
    const db = getDbPool();
    ...
    const guessIds = guessIdRows.map((row) => String(row.id));
    if (guessIds.length === 0) {
        return {
            items: [],
    ...
    return {
        items: sortByIds(typedRows.map((row) => buildGuessSummary(row, optionsByGuess.get(String(row.id)) ?? [], votesByGuess.get(String(row.id)) ?? [])), guessIds),
        total: Number(countRows[0]?.total ?? 0),
        page: safePage,
        pageSize: safePageSize,
    };
...
`;
export async function getAdminUserOrders(userId, page = 1, pageSize = 10) {
    const db = getDbPool();
    const safePage = Math.max(1, page);
    const safePageSize = Math.min(50, Math.max(1, pageSize));
    const offset = (safePage - 1) * safePageSize;
    ...
    const orderIds = orderIdRows.map((row) => String(row.id));
    ...
    `, [orderIds]);
    return {
        items: sortByIds(mapOrderRows(rows), orderIds),
        total: Number(countRows[0]?.total ?? 0),
        page: safePage,
        pageSize: safePageSize,
    };
