...
type RankingSummaryRow = {
  board_type: number | string;
  period_type: number | string;
  period_value: number | string;
  generated_at: Date | string;
  entry_count: number | string;
  top_user_id: number | string | null;
  top_score: number | string | null;
  top_extra_json: string | Record<string, unknown> | null;
  top_user_name: string | null;
  top_user_uid: string | null;
};
...
function mapBoardType(code: number) {
  if (code === BOARD_GUESS_WINS) {
    return 'guessWins' satisfies RankingType;
  }
  if (code === BOARD_INVITE_COUNT) {
    return 'inviteCount' satisfies RankingType;
  }
  return 'winRate' satisfies RankingType;
}

function mapBoardTypeLabel(type: RankingType) {
  if (type === 'guessWins') {
    return '猜中次数榜';
  }
  if (type === 'inviteCount') {
...
function mapPeriodType(code: number) {
  if (code === PERIOD_DAILY) {
    return 'daily' satisfies RankingPeriodType;
  }
  if (code === PERIOD_WEEKLY) {
    return 'weekly' satisfies RankingPeriodType;
  }
  if (code === PERIOD_MONTHLY) {
    return 'monthly' satisfies RankingPeriodType;
  }
  return 'allTime' satisfies RankingPeriodType;
}

function mapPeriodTypeLabel(type: RankingPeriodType) {
  if (type === 'daily') {
    return '日榜';
  }
  if (type === 'weekly') {
...
function formatExtraSummary(type: RankingType, extraJson: Record<string, unknown>) {
  if (type === 'winRate') {
...
function formatPeriodLabel(periodType: RankingPeriodType, periodValue: string) {
...
export async function getAdminRankings(
  filters: AdminRankingFilters,
): Promise<AdminRankingListResult> {
  const db = getDbPool();
  ...
  const items = (rows as RankingSummaryRow[]).map(mapRankingSummaryItem);
...
export async function getAdminRankingDetail(
  boardType: RankingType,
  periodType: RankingPeriodType,
  periodValue: string,
): Promise<AdminRankingDetailResult> {
  const db = getDbPool();
  ...
  const rankingRows = rows as RankingEntryRow[];
