...
import { FRIEND_CONFIRM_CONFIRMED, FRIEND_CONFIRM_REJECTED, GUESS_ACTIVE, GUESS_REJECTED, GUESS_SCOPE_FRIENDS, GUESS_SETTLED, INVITATION_ACCEPTED, INVITATION_EXPIRED, INVITATION_PENDING, INVITATION_REJECTED, fallbackUserName, toIsoString, toNumber, toOptionalNumber, toOptionalStringId, } from './guesses-shared';
function mapFriendGuessStatus(guessStatus, reviewStatus, confirmedResults, rejectedResults, betParticipantCount) {
...
export async function getAdminFriendGuesses() {
    ...
    return {
        items: rows.map((row) => {
            ...
            const status = mapFriendGuessStatus(row.status, row.review_status, confirmedResults, rejectedResults, betParticipantCount);
