...
import { AUTH_SCOPE_ALL_BRAND, AUTH_SCOPE_CATEGORY_ONLY, AUTH_SCOPE_PRODUCT_ONLY, AUTH_STATUS_ACTIVE, AUTH_STATUS_REVOKED, AUTH_TYPE_NORMAL, buildUserDisplayName, createNo, ensurePendingReview, mapAuthScope, mapAuthStatus, mapAuthType, mapReviewStatus, normalizeRejectReason, normalizeReviewStatus, parseJsonValue, PRODUCT_STATUS_ACTIVE, PRODUCT_STATUS_OFF_SHELF, summarizeByKey, toIso, } from './merchant-shared';
function normalizeScopeIdTokens(value) {
    if (value == null) {
        return [];
    }
    ...
    if (typeof value === 'object') {
        ...
        return Object.values(record).flatMap((item) => typeof item === 'object' ? normalizeScopeIdTokens(item) : []);
    }
    return [];
}
function extractScopedAuthIds(scopeValue) {
    return Array.from(new Set(normalizeScopeIdTokens(scopeValue).filter((item) => item.length > 0)));
}
...
export async function revokeAdminBrandAuthRecord(authId) {
    ...
    try {
      ...
      `, [authId]);
        ...
        const scopedIds = authScopeCode === AUTH_SCOPE_ALL_BRAND
            ? []
