...
import { type AdminLiveRoomItem, type AdminLiveRoomListResult, type StopAdminLiveRoomResult } from '@umi/shared';
...
async function fetchAdminLiveRoomById(id: string): Promise<AdminLiveRoomItem | null> {
  ...
  const row = (rows as AdminLiveRoomRow[])[0];
...
export async function stopAdminLiveRoom(id: string): Promise<StopAdminLiveRoomResult> {
  const db = getDbPool();
