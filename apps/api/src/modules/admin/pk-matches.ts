...
import {
  ...
  PK_RESULT_CANCELED,
  PK_RESULT_DRAW,
  PK_RESULT_INITIATOR_WIN,
  PK_RESULT_OPPONENT_WIN,
  PK_RESULT_PENDING,
  PkMatchRow,
  fallbackUserName,
  getOptionTextMap,
...
export interface AdminPkMatchStatsResult {
  total: number;
  pending: number;
  active: number;
  completed: number;
...
function mapPkResultText(row: PkMatchRow, leftUser: string, rightUser: string): string {
  const result = Number(row.result ?? 0);
...
export async function getAdminPkMatchStats(): Promise<AdminPkMatchStatsResult> {
  const rows = await getPkMatchRows();

  return rows.reduce<AdminPkMatchStatsResult>(
    (stats, row) => {
      const status = mapPkMatchStatus(row);
      stats.total += 1;
