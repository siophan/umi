...
import {
  ...
  type AdminCommunityCommentListResult,
  type AdminCommunityPostItem,
  type AdminCommunityPostListResult,
  type AdminCommunityReportItem,
  type AdminCommunityReportListResult,
  type AdminLiveRoomItem,
  type AdminLiveRoomListResult,
  type DeleteAdminCommunityCommentResult,
  type DeleteAdminCommunityPostResult,
...
type CommunityPostListParams = {
  title?: string;
  author?: string;
  tag?: string;
};

type CommunityCommentListParams = {
  content?: string;
  author?: string;
  postTitle?: string;
};
...
function sanitizePost(row: AdminCommunityPostRow): AdminCommunityPostItem {
...
function sanitizeComment(row: AdminCommunityCommentRow): AdminCommunityCommentItem {
  return {
    id: toEntityId(row.id),
    targetType: 'post',
...
function sanitizeReport(row: AdminCommunityReportRow): AdminCommunityReportItem {
...
function sanitizeLiveRoom(row: AdminLiveRoomRow): AdminLiveRoomItem {
...
async function deletePostCascade(connection: mysql.PoolConnection, postId: string) {
  ...
  const [commentRows] = await connection.execute<mysql.RowDataPacket[]>(
    'SELECT id FROM comment_item WHERE target_type = ? AND target_id = ?',
    [COMMENT_TARGET_POST, postId],
  );
  ...
  if (commentIds.length > 0) {
    ...
    await connection.execute(
      'DELETE FROM comment_item WHERE target_type = ? AND target_id = ?',
      [COMMENT_TARGET_POST, postId],
    );
  }
...
async function deleteCommentCascade(connection: mysql.PoolConnection, commentId: string) {
  const [commentRows] = await connection.execute<mysql.RowDataPacket[]>(
    'SELECT id FROM comment_item WHERE id = ? LIMIT 1',
    [commentId],
  );
  ...
  const [relatedRows] = await connection.execute<mysql.RowDataPacket[]>(
    'SELECT id FROM comment_item WHERE id = ? OR parent_id = ?',
    [commentId, commentId],
  );
...
export async function getAdminCommunityPosts(
  ...
  const values: Array<string | number> = [POST_INTERACTION_LIKE, COMMENT_TARGET_POST];
  ...
  return {
    items: (rows as AdminCommunityPostRow[]).map(sanitizePost),
  };
}

export async function deleteAdminCommunityPost(
  postId: string,
): Promise<DeleteAdminCommunityPostResult> {
  const db = getDbPool();
...
export async function getAdminCommunityComments(
  params: CommunityCommentListParams,
): Promise<AdminCommunityCommentListResult> {
  const db = getDbPool();
  ...
  return {
    items: (rows as AdminCommunityCommentRow[]).map(sanitizeComment),
  };
}

export async function deleteAdminCommunityComment(
  commentId: string,
): Promise<DeleteAdminCommunityCommentResult> {
  const db = getDbPool();
  const connection = await db.getConnection();
...
export async function getAdminCommunityReports(
  ...
  appendLikeFilter(
    clauses,
    values,
    'COALESCE(reporter_profile.name, \'\') LIKE ?',
    params.reporter,
  );
...
export async function getAdminLiveRooms(
...
): Promise<AdminLiveRoomListResult> {
  const db = getDbPool();
  const clauses: string[] = [];
  const values: Array<string | number> = [GUESS_STATUS_ACTIVE, GUESS_REVIEW_APPROVED, GUESS_STATUS_ACTIVE, GUESS_REVIEW_APPROVED];
  ...
  const items = (rows as AdminLiveRoomRow[]).map(sanitizeLiveRoom);
