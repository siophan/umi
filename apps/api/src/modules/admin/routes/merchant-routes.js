...
import { createAdminBrand, getAdminBrandAuthApplies, getAdminBrandAuthRecords, getAdminBrands, getAdminShopApplies, getAdminShopDetail, getAdminShopProducts, getAdminShops, reviewAdminBrandAuthApply, reviewAdminShopApply, revokeAdminBrandAuthRecord, updateAdminBrand, updateAdminShopProductStatus, updateAdminShopStatus, } from '../merchant';
import { getRouteParam, toRouteHttpError } from '../route-helpers';
export function registerAdminMerchantRoutes(adminRouter) {
    ...
    adminRouter.put('/shops/products/:id/status', asyncHandler(async (request, response) => {
        try {
            ok(response, await updateAdminShopProductStatus(getRouteParam(request.params.id), request.body));
        }
        catch (error) {
            throw toRouteHttpError(error, {
                status: 400,
