...
import type { UploadOssImagePayload, UploadOssVideoPayload } from '@umi/shared';

import { asyncHandler } from '../../../lib/errors';
import { ok } from '../../../lib/http';
import { uploadOssImage, uploadOssVideo } from '../../upload/oss-upload';
...
export function registerAdminUploadRoutes(adminRouter: ExpressRouter) {
  ...
  adminRouter.post(
    ...
    asyncHandler(async (request, response) => {
      const admin = getRequestAdmin(request);
      ok(response, await uploadOssVideo(request.body as UploadOssVideoPayload, admin.id));
    }),
  );
}
