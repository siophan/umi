...
import type { AdminLoginPayload, ChangePasswordPayload } from '@umi/shared';
...
export function registerAdminPublicAuthRoutes(adminRouter: ExpressRouter) {
  adminRouter.post(
    '/auth/login',
...
export function registerAdminSessionRoutes(adminRouter: ExpressRouter) {
  adminRouter.get(
    '/auth/me',
    asyncHandler(async (request, response) => {
