...
export function registerAdminContentRoutes(adminRouter) {
    ...
    adminRouter.get('/guesses/:id', asyncHandler(async (request, response) => {
        try {
    ...
    adminRouter.post('/guesses', asyncHandler(async (request, response) => {
        try {
            ok(response, await createAdminGuess(request.body, getRequestAdmin(request).id));
        }
        catch (error) {
            throw toRouteHttpError(error, {
                status: 400,
    ...
    adminRouter.get('/pk/stats', asyncHandler(async (_request, response) => {
