...
function trimOptionalText(value: string | null | undefined, maxLength = 255) {
...
function normalizeCouponTemplateStatus(
  value:
...
function mapCouponType(type: number | string | null | undefined): AdminCouponTemplateItem['type'] {
...
function mapCouponValidityType(
...
function sanitizeCouponTemplate(row: CouponTemplateRow): AdminCouponTemplateItem {
...
function sanitizeCouponGrantBatch(row: CouponGrantBatchRow): AdminCouponGrantBatchItem {
...
function mapCouponTypeCode(type: CreateAdminCouponTemplatePayload['type']) {
  if (type === 'discount') {
...
function normalizeCouponTemplatePayload(
...
async function assertShopExists(connection: mysql.PoolConnection, shopId: string) {
...
function buildCouponListWhere(params: CouponListParams) {
...
async function fetchCouponTemplateById(connection: mysql.PoolConnection, templateId: string) {
  const [rows] = await connection.execute<mysql.RowDataPacket[]>(
    `
      SELECT
        ct.id,
...
function buildCouponAmountForIssuedCoupon(template: CouponTemplateRow) {
...
async function fetchCouponRecipientIds(
...
function getCouponGrantExpiryAt(template: CouponTemplateRow) {
