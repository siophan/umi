...
function normalizeBizTypeCode(code) {
    const value = Number(code ?? 0);
...
function buildCategoryPath(parentPath, categoryId) {
    return parentPath ? `${parentPath}/${categoryId}` : String(categoryId);
}
async function findCategoryBaseById(db, categoryId) {
...
async function requireCategory(db, categoryId) {
...
async function resolveParentCategory(db, parentId, bizTypeCode) {
    if (!parentId) {
        return null;
    }
...
export async function getAdminCategories() {
    ...
    const byBizType = items.reduce((summary, item) => {
