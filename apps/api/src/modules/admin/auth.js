...
function resolveAdminRoutePermissionCodes(method, path) {
...
export async function requireAdminRoutePermission(request, _response, next) {
    try {
        const admin = getRequestAdmin(request);
        const requiredCodes = resolveAdminRoutePermissionCodes(request.method, request.path);
