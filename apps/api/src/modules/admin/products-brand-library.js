...
import { BRAND_STATUS_ACTIVE, buildAdminBrandLibraryFilters, normalizeAdminBrandProductPayload, normalizeBrandProductStatus, normalizePage, normalizePageSize, PRODUCT_STATUS_ACTIVE, sanitizeAdminBrandLibrary, toNumber, } from './products-shared';
...
export async function createAdminBrandProduct(payload) {
...
export async function updateAdminBrandProduct(brandProductId, payload) {
    const normalized = normalizeAdminBrandProductPayload(payload);
    const db = getDbPool();
    const connection = await db.getConnection();
    try {
      ...
      `, [normalized.categoryId]);
        ...
        const isCurrentCategory = String(productRows[0].category_id ?? '') === String(normalized.categoryId);
