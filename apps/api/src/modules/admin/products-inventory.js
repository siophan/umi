...
import { BRAND_PRODUCT_STATUS_ACTIVE, BRAND_PRODUCT_STATUS_DISABLED, BRAND_STATUS_ACTIVE, BRAND_STATUS_DISABLED, buildAdminProductFilters, LOW_STOCK_THRESHOLD, normalizePage, normalizePageSize, PRODUCT_STATUS_ACTIVE, PRODUCT_STATUS_DISABLED, PRODUCT_STATUS_OFF_SHELF, sanitizeAdminProduct, SHOP_STATUS_ACTIVE, SHOP_STATUS_PAUSED, toNumber, } from './products-shared';
export async function getAdminProducts(query = {}) {
    ...
    return {
        ...
        pageSize,
        summary: {
            total: toNumber(summaryRows[0]?.total),
            byStatus: {
                active: toNumber(summaryRows[0]?.active_count),
                low_stock: toNumber(summaryRows[0]?.low_stock_count),
                paused: toNumber(summaryRows[0]?.paused_count),
                off_shelf: toNumber(summaryRows[0]?.off_shelf_count),
                disabled: toNumber(summaryRows[0]?.disabled_count),
            },
        },
    };
}
