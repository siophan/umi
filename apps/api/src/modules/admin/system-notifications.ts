...
import {
  mapNotificationAudience,
  mapNotificationAudienceTargetType,
  mapNotificationTargetType,
  mapNotificationType,
  NOTIFICATION_TARGET_CHAT,
  NOTIFICATION_TARGET_GUESS,
  NOTIFICATION_TARGET_ORDER,
  NOTIFICATION_TARGET_POST,
...
export type { AdminNotificationAudience, AdminNotificationStatus, AdminNotificationType };
...
type AdminNotificationListQuery = {
  ...
  keyword?: string;
  type?: AdminNotificationType;
  audience?: Exclude<AdminNotificationAudience, 'targeted_users'>;
};
...
function mapNotificationTypeCode(type: AdminNotificationType | undefined) {
  if (type === 'order') return NOTIFICATION_TYPE_ORDER;
  if (type === 'guess') return NOTIFICATION_TYPE_GUESS;
  if (type === 'social') return NOTIFICATION_TYPE_SOCIAL;
  if (type === 'system') return NOTIFICATION_TYPE_SYSTEM;
  return null;
}

function buildNotificationWhereClause(query: AdminNotificationListQuery) {
...
async function fetchAdminNotificationBatches(query: AdminNotificationListQuery) {
  const db = getDbPool();
  ...
  return {
    items: rows as AdminNotificationRow[],
    summary: (countRows[0] ?? {
      total: 0,
      sent: 0,
      read: 0,
      unread: 0,
    }) as AdminNotificationSummaryRow,
    page,
    pageSize,
  };
}
...
export async function getAdminNotifications(
  ...
  const items = result.items.map<AdminNotificationItem>((row) => ({
  ...
  return {
    ...
    basis:
      '基于 notification 真表按消息载荷聚合为发送批次视图；搜索、筛选和分页都建立在真实批次全集上。',
  };
}
