...
import { COMMENT_TARGET_POST, POST_INTERACTION_BOOKMARK, POST_INTERACTION_LIKE, buildPostVisibilityClause, } from './constants';
export async function buildCommunityGuessInfoMap(rows) {
    const guessIds = Array.from(new Set(rows
        .map((row) => Number(row.guess_id ?? 0))
        .filter((value) => Number.isFinite(value) && value > 0)));
    const guessInfoMap = new Map();
    if (!guessIds.length) {
        return guessInfoMap;
    }
    const db = getDbPool();
    ...
    for (const row of voteRows) {
        const key = String(row.guess_id);
        const list = voteMap.get(key) ?? new Map();
    ...
    for (const guessId of guessIds) {
        ...
        const votes = voteMap.get(String(guessId)) ?? new Map();
...
export async function fetchCommunityFeedRows(userId, tab) {
    const db = getDbPool();
...
export async function fetchCommunityPostRow(userId, postId) {
    const db = getDbPool();
    const visibilitySql = buildPostVisibilityClause('p');
    const [rows] = await db.execute(`
      SELECT
