...
async function ensureUserUidCode(userId, currentCode) {
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
        ...
        `, [candidate, userId]);
            ...
            const [rows] = await db.execute(`SELECT uid_code FROM user WHERE id = ? LIMIT 1`, [userId]);
...
async function attachUserUidCode(row) {
    if (!row) {
        return null;
    }
...
export async function findUserByPhone(phone) {
    ...
    const [rows] = await db.execute(`
      ...
      LIMIT 1
    `, [phone]);
    return attachUserUidCode(rows[0] ?? null);
}
...
async function findUserByUidCode(uidCode) {
    const db = getDbPool();
    const [rows] = await db.execute(`
      ${userSelectSql}
      WHERE u.uid_code = ?
...
export async function findUserByPublicKey(key) {
...
export async function getUserSummaryById(userId) {
