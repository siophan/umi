...
function buildAdminUserFilterWhere(query) {
    const conditions = [];
    const params = [];
    const keyword = query.keyword?.trim();
    if (keyword) {
        const like = `%${keyword}%`;
        conditions.push(`(source.name LIKE ? OR source.uid_code LIKE ? OR source.phone_number LIKE ? OR source.shop_name LIKE ? OR source.region LIKE ?)`);
...
`;
async function attachAdminRows(rows) {
    return Promise.all(rows.map((row) => findUserById(row.id)));
}
