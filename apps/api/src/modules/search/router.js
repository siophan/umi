...
function buildFeedTag(sourceTags, details) {
...
async function searchProducts(query, sort, limit, userId) {
    const db = getDbPool();
    const like = `%${query}%`;
    const whereClauses = [
        'p.status = 10',
...
async function getGuessOptionsAndVotes(guessIds) {
    if (!guessIds.length) {
        return {
            optionsByGuess: new Map(),
            votesByGuess: new Map(),
        };
    }
    const db = getDbPool();
    ...
    for (const option of optionRows[0]) {
...
async function searchGuesses(query, limit) {
    const db = getDbPool();
    const like = `%${query}%`;
    ...
    return {
        total: Number(countRows[0][0]?.total ?? 0),
        items: resultRows.map((row) => buildGuessSummary(row, optionsByGuess.get(String(row.id)) || [], votesByGuess.get(String(row.id)) || [])),
    };
}
async function getHotSearches(limit) {
    const db = getDbPool();
...
async function getSearchSuggestions(query, limit) {
    ...
    const items = [
        ...productRows[0].map((row) => ({ text: row.text, type: 'product' })),
        ...brandRows[0].map((row) => ({ text: row.text, type: 'brand' })),
        ...guessRows[0].map((row) => ({ text: row.text, type: 'guess' })),
    ]
