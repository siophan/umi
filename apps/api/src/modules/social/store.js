...
function sanitizeSocialRow(row) {
...
function sanitizeUserSearchRow(row) {
    const totalGuess = Number(row.total_guess ?? 0);
    const wins = Number(row.wins ?? 0);
    return {
        id: toEntityId(row.id),
...
export async function getSocialOverview(userId) {
  ...
  `;
    ...
    return {
        friends: friendRows.map((row) => sanitizeSocialRow(row)),
        following: followingRows.map((row) => sanitizeSocialRow(row)),
        fans: fanRows.map((row) => sanitizeSocialRow(row)),
        requests: requestRows.map((row) => sanitizeSocialRow(row)),
    };
}
export async function acceptFriendRequest(viewerId, requesterId) {
    const db = getDbPool();
...
export async function rejectFriendRequest(viewerId, requesterId) {
    const db = getDbPool();
    const [pendingRows] = await db.execute(`
      SELECT id
      FROM friendship
...
export async function searchUsers(userId, q) {
    ...
    return {
        items: rows.map((row) => sanitizeUserSearchRow(row)),
    };
}
