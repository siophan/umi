...
import { createNo, getCurrentShop, STATUS_ACTIVE, STATUS_APPROVED, STATUS_PENDING } from './shop-shared';
export async function getBrandAuthOverview(userId) {
    const db = getDbPool();
...
export async function submitBrandAuthApplication(userId, input) {
    const shop = await getCurrentShop(userId);
    if (!shop) {
    ...
    if (existing && [STATUS_PENDING, STATUS_APPROVED].includes(Number(existing.status))) {
        throw new HttpError(400, Number(existing.status) === STATUS_APPROVED ? 'SHOP_BRAND_AUTH_ALREADY_APPROVED' : 'SHOP_BRAND_AUTH_PENDING', Number(existing.status) === STATUS_APPROVED ? '该品牌已授权' : '该品牌已在审核中');
    }
...
export async function getBrandProducts(userId, brandId) {
    if (!brandId) {
...
export async function addShopProducts(userId, input) {
    const brandId = typeof input.brandId === 'string' ? input.brandId.trim() : '';
    const brandProductIds = Array.isArray(input.brandProductIds)
        ? input.brandProductIds.filter((item) => typeof item === 'string' && item.trim().length > 0)
        : [];
