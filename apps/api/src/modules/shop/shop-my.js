...
import { createNo, getCurrentShop, getLatestShopApplication, listShopCategories, STATUS_ACTIVE, STATUS_APPROVED, STATUS_PENDING, toShopStatusResult, } from './shop-shared';
/**
 * 我的店铺页需要店铺概览、品牌授权和在售商品三块数据，这里集中做一次聚合查询。
 */
export async function getMyShopResult(userId) {
    const db = getDbPool();
...
export async function getMyShopStatus(userId) {
...
export async function submitShopApplication(userId, input) {
