...
import { FULFILLMENT_PENDING, FULFILLMENT_PROCESSING, FULFILLMENT_SHIPPED, getFavoritedProductIdSet, getProductById, GUESS_ACTIVE, mapGuessReviewStatus, mapGuessStatus, PHYSICAL_STATUS_CONSIGNING, PHYSICAL_STATUS_STORED, PRODUCT_CATEGORY_BIZ_TYPE, REVIEW_APPROVED, safeJsonArray, sanitizeWarehouseRow, VIRTUAL_STATUS_LOCKED, VIRTUAL_STATUS_STORED, } from './product-shared';
...
export async function getProductDetail(productId, userId) {
    ...
    const [activeGuess, warehouseItems, recommendations, favoritedProductIds, reviews] = await Promise.all([
        ...
        userId ? getFavoritedProductIdSet(userId, [productId]) : Promise.resolve(new Set()),
