...
function mapOrderDetail(rows, logs) {
...
function parseCouponCondition(condition) {
...
orderRouter.post('/:id/urge', requireUser, withErrorBoundary({
    ...
    const db = getDbPool();
    const [rows] = await db.execute(`SELECT id, status FROM \`order\` WHERE id = ? AND user_id = ? LIMIT 1`, [orderId, user.id]);
    const row = rows[0];
    if (!row?.id) {
        throw new HttpError(404, 'ORDER_NOT_FOUND', '订单不存在');
    }
    ...
    await db.execute(`INSERT INTO order_status_log (order_id, from_status, to_status, operator_id, operator_role, note, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP(3), CURRENT_TIMESTAMP(3))`, [orderId, row.status, row.status, user.id, 'user', '用户催发货']);
...
orderRouter.post('/:id/review', requireUser, withErrorBoundary({
    ...
    const [rows] = await db.execute(`SELECT o.id FROM \`order\` o
         ...
         LIMIT 1`, [productId, orderId, user.id, ORDER_FULFILLED]);
    if (!rows.length) {
    ...
    await db.execute(`INSERT INTO product_review (order_id, user_id, product_id, rating, content, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP(3), CURRENT_TIMESTAMP(3))
         ON DUPLICATE KEY UPDATE rating = VALUES(rating), content = VALUES(content), updated_at = CURRENT_TIMESTAMP(3)`, [orderId, user.id, productId, ratingNum, content?.trim() || null]);
    ok(response, { success: true });
}));
