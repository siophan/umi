...
1. `apps/api` 仍有明显未完成区域，尤其是 Admin、更多交易写接口和部分次级业务页。
2. `apps/web` 仍有不少 demo / mock 或前端派生内容，主要集中在直播、首页部分资源位和商品详情局部说明区。
...
当前状态：进行中（认证、竞猜、商品发现、购物、支付下单、订单、仓库、店铺等主链路已开始接库，Admin 和更多交易写接口仍待推进）
