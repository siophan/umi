...
3. 持续把商品发现、商品详情、搜索、购物车等高频读链路维持在真实数据态；
...
- `/cart`：已切真实 `cart_item` 读写，支持读取购物车、勾选、改数量、删除和从推荐商品加入购物车
...
3. 在接真实接口时，优先保留旧页 fallback 和旧页交互，不要为了联调破坏 UI；`/mall` 当前主商品流依赖真实 `/api/products`，`/cart` 当前主链路依赖真实 `/api/cart`
