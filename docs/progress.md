...
| 用户端 `apps/web` | `进行中` | 旧静态页已 `41/41` 覆盖，用户端页面已完成一轮总复查；除登录页仍由其他线程单独处理外，其余页面的残留差异已基本收平，当前主要剩真实接口替换与零散文案维护 |
...
| 功能 | 状态 | 说明 |
| --- | --- | --- |
| 首页竞猜流 | `进行中` | 首页结构、榜单、开奖区、底部导航已按旧页持续收口，仍以静态展示为主 |
| 商城首页 | `进行中` | 已有页面和旧 UI 结构，商城卡片和横幅已对齐旧页，商品发现流仍未系统接真实接口 |
| 登录 / 注册 UI | `已完成` | 页面已完成且已接真实认证接口，支持验证码 / 密码登录、注册、登录态持久化 |
| 个人中心 UI | `进行中` | `me/profile/edit-profile` 已接真实资料、作品、收藏、点赞和消息未读数；优米号已改为后端生成的唯一 `uid_code`；设置抽屉、搜索层、开店弹窗已继续对齐旧页 |
| 竞猜详情 UI | `进行中` | 页面主体、PK 进度条、评论、分享、下注弹层已继续对齐旧页；竞猜主数据已切到真实接口，评论/弹幕/下注闭环仍未完成 |
| 商品详情 UI | `进行中` | `product/[id]` 已接真实商品主数据、进行中竞猜、换购库存和推荐商品；评价、部分玩法说明和周边购买链路仍是静态展示 |
| 店铺详情 UI | `进行中` | 店铺 Hero、活动、商品卡、竞猜入口、聊天/收藏/分享交互已继续收口，仍是静态数据 |
...
| 社区 / 聊天 / 用户主页 UI | `进行中` | `user/[id]`、`friends`、`notifications`、`chat`、`chat/[id]` 已接真实接口且已清主要 fallback；社区流本身仍以 mock 为主 |
| 搜索 UI | `进行中` | 结果页已做，返回行为与旧页一致，数据仍为本地构造 |
| 消息 / 通知 UI | `进行中` | 已补老页 fallback，系统入口、列表结构、时间分隔、已读交互已继续对齐旧页；真实 `notification/chat` 读写仍在持续接入 |
...
| 邀请 / 编辑资料 / 调试页 UI | `进行中` | `invite`、`edit-profile`、`test-api` 已继续清理开发态内容并对齐旧页结构 |
...
| 页面调用本地 demo 数据 | `进行中` | 主要残留在商城发现流、`shop/[id]`、社区/直播/支付等周边页面；`product/[id]` 已切真实主数据 |
| 页面接真实后端接口 | `进行中` | 已覆盖 auth / me / friends / notifications / user profile / chat / shop / orders / warehouse / guess-history / guess-detail / product-detail 等模块 |

一句话：`apps/web` 当前是“旧静态页面已 41/41 全覆盖，高频页面 UI/交互已经过多轮对齐，认证和个人中心相关链路已接真实接口，但核心业务数据仍大量未接线”。 
...
| 真实数据库接入 | `进行中` | 认证、用户资料、好友、通知、聊天、钱包、竞猜、订单、仓库、店铺、商品详情已接当前 MySQL；Admin 与部分商城/社区接口仍未切库 |
...
一句话：`apps/api` 当前是“认证和用户中心已形成最小真实后端，业务主链路仍大量是 demo”。
...
| 优先级 | 缺口 | 说明 |
| --- | --- | --- |
| P0 | 商品发现链路仍未完全接真实 API | 商城首页、搜索、店铺详情仍有大量静态页面，`product/[id]` 仍有部分静态展示块待清理 |
...
| P2 | 文案 / UI / 页面行为仍在持续对齐旧系统 | 页面多，细节仍需继续压平 |

## 下一阶段建议

1. 继续把商城首页、搜索、店铺详情切到真实商品接口，并收完 `product/[id]` 剩余静态块。
