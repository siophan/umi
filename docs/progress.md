...
| 用户端 `apps/web` | `进行中` | 页面覆盖很完整，UI/交互对齐已持续推进，但大多仍是静态 / Demo |
...
| 消息 / 通知 UI | `进行中` | `chat`、`chat/[id]`、`notifications` 已持续对齐旧系统，仍为本地数据 |
| 个人店铺 / 上架商品 UI | `进行中` | `my-shop`、`add-product` 已收口旧页结构和返回行为 |
| 邀请 / 编辑资料 / 调试页 UI | `进行中` | `invite`、`edit-profile`、`test-api` 已继续按旧静态页收口 |
