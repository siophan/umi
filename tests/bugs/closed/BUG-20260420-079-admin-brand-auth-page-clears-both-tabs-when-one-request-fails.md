...
| 项目 | 内容 |
| --- | --- |
| 修复说明 | 已把品牌授权页的“授权审核列表”和“授权记录”拆成各自独立失败处理：页面初始化改成 `Promise.allSettled`，任意一侧接口失败时只影响对应数据集，并分别暴露局部 warning，不再把另一侧已成功数据一起清空。 |
| 验证命令 | `pnpm --filter @umi/admin typecheck`；`pnpm --filter @umi/admin build` |
| Fixer 自测结果 | 通过。品牌授权页现在可以在单侧接口失败时保留另一侧真实数据；admin `typecheck` 与 `build` 均通过。 |
| Verifier 复测结果 | `2026-04-21` 代码复核通过。当前 [apps/admin/src/pages/shop-brand-auth-applies-page.tsx](/Users/ezreal/Downloads/joy/umi/apps/admin/src/pages/shop-brand-auth-applies-page.tsx:65) 到 [apps/admin/src/pages/shop-brand-auth-applies-page.tsx](/Users/ezreal/Downloads/joy/umi/apps/admin/src/pages/shop-brand-auth-applies-page.tsx:95) 已改为 `Promise.allSettled`，并按单侧失败分别保留另一侧真实数据与局部 warning。 |
| 修复提交/变更 | `/Users/ezreal/Downloads/joy/umi/apps/admin/src/pages/shop-brand-auth-applies-page.tsx` |
...
| `review_mode` | `代码验证` |
| `结论` | `已复核通过` |
| `说明` | `2026-04-21` 按 `#/shops/brand-auth` 同页合并复核时重新核对当前工作树：[apps/admin/src/pages/shop-brand-auth-applies-page.tsx:53](/Users/ezreal/Downloads/joy/umi/apps/admin/src/pages/shop-brand-auth-applies-page.tsx:53) 到 [apps/admin/src/pages/shop-brand-auth-applies-page.tsx:88](/Users/ezreal/Downloads/joy/umi/apps/admin/src/pages/shop-brand-auth-applies-page.tsx:53) 已改为 `Promise.allSettled`，并分别保留单侧成功数据与局部 warning；原问题已修复。 |
