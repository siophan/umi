...
| 项目 | 内容 |
| --- | --- |
| 修复说明 | 移除了店铺详情成功态里本地推导的评分卡、服务分和开店时间，不再把 `avgRating` 二次包装成“商品质量 / 物流速度 / 服务态度”，也不再用商品创建时间最小值冒充“开店时间”。页面只保留真实店铺字段和基于真实商品/竞猜列表的明确聚合值。 |
...
| Fixer 自测结果 | 通过。`/shop/[id]` 已不再展示伪评分卡和伪开店时间，只保留真实店铺字段与明确聚合信息。 |
| Verifier 复测结果 | 待补 |
